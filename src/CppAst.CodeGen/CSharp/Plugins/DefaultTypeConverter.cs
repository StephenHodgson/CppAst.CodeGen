                    ...
                    if (!nested && (isParam || isReturn))
                    {
                        switch (elementType.TypeKind)
                        {
                            ...
                            case CppTypeKind.Qualified:
                                ...
                                csType = new CSharpRefType(qualifiedType.Qualifier == CppTypeQualifier.Const
                                    ? (isParam ? CSharpRefKind.In : CSharpRefKind.RefReadOnly)
                                    : CSharpRefKind.Ref, converter.GetCSharpType(qualifiedType.ElementType, context, true));
                                break;
                            ...
                            case CppTypeKind.StructOrClass:
                                ...
                                {
                                    csType = new CSharpRefType(isConst
                                        ? (isParam ? CSharpRefKind.In : CSharpRefKind.RefReadOnly)
                                        : CSharpRefKind.Ref, pointedCSharpType);
                                }
                            ...
                            case CppTypeKind.Primitive:
                                ...
                                if (cppPrimitive.Kind != CppPrimitiveKind.Void &&
                                    cppPrimitive.Kind != CppPrimitiveKind.Char &&
                                    cppPrimitive.Kind != CppPrimitiveKind.UnsignedChar)
                                {
                                    csType = new CSharpRefType(CSharpRefKind.Ref, pointedCSharpType);
                                }
                    ...
                    case CppTypeKind.Array:
                        ...
                        if (arrayType.Size < 0 && arrayElementType.Equals(ConstChar))
                        {
                            ...
                            csType = GetStringType(converter);
                                 ...
                                 (arrayElementType.Equals(CppPrimitiveType.Char) ||
                                  arrayElementType.Equals(CppPrimitiveType.WChar)))
                        {
                            var fixedStrType = new CSharpTypeWithAttributes(CSharpPrimitiveType.String);
                            fixedStrType.Attributes.Add(new CSharpMarshalAttribute(CSharpUnmanagedKind.ByValTStr)
                            {
                                SizeConst = arrayType.Size
                            });
                            csType = fixedStrType;
                        }
                        else
                        {
                            if (converter.Options.AllowFixedSizeBuffers &&
                                ...
                                arrayElementType.GetCanonicalType() is CppPrimitiveType cppPrimitive &&
