...
namespace CppAst.CodeGen.CSharp
{
    public class DefaultTypedefConverter : ICSharpConverterPlugin
    {
        ...
        private CSharpElement ConvertTypedef(CSharpConverter converter, CppTypedef cppTypedef, CSharpElement context)
        {
            ...
            bool isSizeT = isFromSystemIncludes && cppTypedef.Name == "size_t";

            if (noWrap || isFromSystemIncludes && elementType.TypeKind != CppTypeKind.Pointer && !isSizeT)
            {
            ...
            var csElementTypeName = isSizeT
                ? "IntPtr"
                : converter.ConvertTypeReferenceToString(csElementType, out attachedAttributes);
